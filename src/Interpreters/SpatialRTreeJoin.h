...
public:
    ...
    /// Only the build (right) side is accounted: the R-tree is built from it, while the
    /// probe side streams through `joinBlock` without per-side counters to report.
    StepAnalysisReport getAnalysisReport() const override
    {
        StepAnalysisReport report;
        MetricList right_metrics = joinSideMetrics(total_right_rows, std::nullopt);
        right_metrics.emplace_back(MetricKey::Size, total_right_bytes);
