...
///   4. COL_COMPLEX decoder: Array(Tuple(UInt64, Float64)) with manually-crafted binary buffer
...
//
// Manually construct a COLUMNAR_V1 buffer (sequential layout) for 2 rows:
//   row 0: [(10, 1.5), (20, 2.5)]
//   row 1: [(30, 3.5)]
//
// COLUMNAR_V1 layout (data section, sequential from data_offset):
...
TEST(ColumnarV1Wire, DecodeArrayOfTupleUInt64Float64)
{
    ...
    // Descriptor — canonical layout: offsets_offset → outer offset array (num_rows+1 uint32s),
    //              data_offset → nested complex data (writeComplexData format).
    ColDescriptor desc{};
    desc.type           = COL_COMPLEX;
    desc.offsets_offset = data_off;
    desc.data_offset    = data_off + outer_bytes;
    desc.data_size      = u64_bytes + f64_bytes;
    std::memcpy(buf.data() + COLUMNAR_HEADER_BYTES, &desc, COLUMNAR_DESC_BYTES);
...
TEST(ColumnarV1Wire, VariantUInt64String)
{
    ...
    EXPECT_EQ(inner0.null_offset, 2u);  // repurpose null_offset to carry sub_rows for the decoder
    ...
    EXPECT_EQ(inner1.null_offset, 1u);  // repurpose null_offset to carry sub_rows for the decoder
