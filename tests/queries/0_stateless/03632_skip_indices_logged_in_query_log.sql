...
SELECT * FROM children FORMAT Null SETTINGS log_comment='1', use_skip_indexes_on_data_read=0; -- no skip indexes used
SELECT * FROM children WHERE age = 3 FORMAT Null SETTINGS log_comment='2', use_skip_indexes_on_data_read=0; -- age_i used
SELECT * FROM children WHERE hasToken(name, 'Alice') FORMAT Null SETTINGS log_comment='3', use_skip_indexes_on_data_read=0; -- name_i used
SELECT * FROM children WHERE age = 3 AND name = 'Alice' FORMAT Null SETTINGS log_comment='4', use_skip_indexes_on_data_read=0; -- age_i and name_i used
SELECT * FROM children
LEFT JOIN parents ON children.parent_id = parents.uid
WHERE parents.age >= 25 AND children.name = 'Alice'
FORMAT Null SETTINGS log_comment='5', use_skip_indexes_on_data_read=0; -- age_i and name_i used
...
SELECT * FROM `hello, world` WHERE age = 30 FORMAT Null SETTINGS log_comment='6', use_skip_indexes_on_data_read=0; -- my.favorite.index used
